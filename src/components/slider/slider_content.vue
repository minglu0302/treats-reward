...
<template>
    <div class='slider-container' :class = 'basicdata.containerClass' :style="noMarginTop" >
       ...
       <template  v-if="!sliderinit.loop">
        <template  v-for="(item, index) in pages">
      	 <div class="slider-item" :style="item.style">
            <div class="slider-item-container" v-bind:class="{bgColor1 : (contentType=='0') }">
              <div v-if="contentType=='0' "> 
                  <div v-if="index=='0'"> 
                      <alertNodata v-if="datas.all.total_rows === 0" > </alertNodata>
                      <HistoryListItem v-for="dataItem in datas.all" :item="dataItem" v-on:showAlert="showAlert"></HistoryListItem> 
                  </div>
                  <div v-if="index=='1'">
                      <alertNodata v-if="datas.available.total_rows === 0" > </alertNodata>
                      <HistoryListItem v-for="dataItem in datas.available" :item="dataItem"  v-on:showAlert="showAlert"></HistoryListItem> 
                  </div>
                  <div v-if="index=='3'">
                      <alertNodata v-if="datas.expired.total_rows === 0" > </alertNodata>
                      <HistoryListItem v-for="dataItem in datas.expired" :item="dataItem"  v-on:showAlert="showAlert"></HistoryListItem> 
                  </div>
              </div>
              <div v-if="contentType=='1'">
                <div class="description-box">
                      <h1>Features</h1>
                      <p>Now you both can rest easy. <br><br> Keeping track of what your pets are up to while you’re not with them is not just interesting; it’s imperative to keeping them healthy. A dip in activity, a spike in sleep, or too many calories per day are all signs it’s time to take your pal in for a checkup or change their routine. Take the guess work out with Bean.</p>
                      <h1>Product Description</h1>
                      
                      <section>
                          <table class="tg"> 
                              <tr class="tg-title-box">
                                  <td class="tg-title">
                                      DATA COLLECTING
                                  </td>
                              </tr>
                              <tr> 
                                  <td>
                                      Dual-Core structure with proprietary algorithm for data analyzing and power saving
                                  </td>
                              </tr>
                              <tr class="tg-title-box">
                                  <td class="tg-title">
                              ...
                              <tr>
                                  <td>
                                      Activity and resting level <br> Calories burned & feeding suggestion<br> Bluetooth mesh-network based Lost &<br> Found Community Alert
                                  </td> 
                              </tr>
                              <tr class="tg-title-box">
                                  <td class="tg-title">
                              ...
                              <tr>
                                  <td>
                                      Data synchronization: Up to 30ft <br>Community Lost Pet Alert signal: up to 30ft
                                  </td> 
                              </tr>
                              <tr class="tg-title-box">
                                  <td class="tg-title">
...
<script>
import alertNodata from '../alertNodata.vue';
import HistoryListItem from '../HistoryListItem.vue'; 

export default {
     props: ['datas', 'alldata', 'sliderinit', 'pages', 'contentType'],
     data(){
     	return {
        noMarginTop : {
            marginTop : '10px'
     ...
     methods:{
      showAlert () {
        ...
        swipeMove (e) { 
            if ( window.document.getElementsByClassName('slider-item-container')[0].scrollTop >= 1 ){
                this.noMarginTop = {
                    marginTop : '0px'
                }
            } else {
